...
`type: ALL`은 전체 테이블 스캔, `key: NULL`은 인덱스를 사용하지 않았다는 뜻이다. `Using filesort`는 결과를 정렬하기 위해 추가 작업이 필요하다는 의미다. 이 출력의 각 항목이 무엇을 의미하는지는 이후 `EXPLAIN` 편에서 다룬다.
...
데이터베이스는 파일 시스템 위에 동시성 제어, 장애 복구, 효율적 검색을 얹은 시스템이다. MySQL은 서버 레이어와 스토리지 엔진 레이어로 나뉘며, 두 레이어가 handler API로 통신한다. SQL 한 줄이 파싱, 최적화, 실행, 저장소 접근이라는 네 단계를 거쳐 결과가 된다.

이 구조를 머릿속에 두면 앞으로의 내용이 어디에 위치하는지 파악하기 쉽다. 인덱스는 스토리지 엔진 레이어의 주제이고, 실행 계획은 옵티마이저의 출력이고, 트랜잭션은 서버 레이어와 스토리지 엔진 양쪽에 걸쳐 있다.
