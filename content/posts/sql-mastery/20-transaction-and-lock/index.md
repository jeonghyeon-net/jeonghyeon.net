...
트랜잭션은 ACID 원칙을 보장하는 작업 단위다. InnoDB는 undo log로 원자성을, redo log로 지속성을, MVCC와 락으로 격리성을 구현한다. 행 수준 락은 인덱스 레코드에 걸리며, 갭 락은 phantom read를 방지한다. 데드락은 완전히 피할 수 없지만, 일관된 접근 순서와 짧은 트랜잭션으로 빈도를 줄일 수 있다. 트랜잭션과 락은 정확성과 성능 사이의 균형을 잡는 도구이며, 이 균형은 격리 수준 설정으로 조절할 수 있다.
