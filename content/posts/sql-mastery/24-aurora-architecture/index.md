...
InnoDB에서 트랜잭션을 커밋할 때 어떤 일이 일어나는지 복습하자. 하나의 UPDATE 문이 커밋되면 다음 쓰기가 발생한다:
...
기존 MySQL에서 UPDATE 한 건의 커밋 과정을 네트워크 관점에서 더 자세히 비교해보자.
...
Aurora의 핵심 아이디어는 간단하다: redo log만 네트워크로 보내고, 나머지는 스토리지 레이어가 알아서 처리하게 한다. 이 단순한 원칙이 네트워크 트래픽 감소, 복제 단순화, 빠른 장애 복구라는 연쇄적인 이점을 만들어낸다.

이 아키텍처를 이해한 위에서, 다음으로 스토리지 레이어의 내부 동작 — quorum 복제, protection group, crash recovery — 을 구체적으로 살펴본다.
