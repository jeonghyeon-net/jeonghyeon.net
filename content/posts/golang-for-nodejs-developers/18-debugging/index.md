...
Node.js에서 별도의 디버거를 설치할 필요가 없는 것과 다르다. Node.js는 V8 inspector가 내장되어 있어 `node --inspect`만으로 디버깅을 시작할 수 있다. Go는 delve를 명시적으로 설치해야 한다.
...
CLI 디버거는 강력하지만, 실제 개발에서는 IDE 연동이 훨씬 편하다. Node.js 개발자가 Chrome DevTools보다 VS Code 디버거를 더 많이 쓰는 것과 같은 이유다.
...
Node.js에서 VS Code 디버깅을 해봤다면 Go에서도 동일한 경험을 얻는다. launch.json의 `type`이 `"node"`에서 `"go"`로 바뀌는 것 정도가 차이다.
...
goroutine 간에 자유롭게 전환하면서 각각의 상태, 지역 변수, 스택 트레이스를 확인할 수 있다. 이것은 Node.js 디버깅에는 없는 개념이다.

VS Code에서도 CALL STACK 패널에 goroutine이 각각 표시된다. goroutine을 클릭하면 해당 goroutine의 스택과 변수를 볼 수 있다.
...
| 개념 | Node.js | Go |
|---|---|---|
| 디버거 | V8 inspector (내장) | delve (별도 설치) |
| 디버깅 시작 | `node --inspect-brk` | `dlv debug` |
| GUI 디버거 | Chrome DevTools | IDE (VS Code, GoLand) |
| breakpoint | 소스 맵 기반 | 소스 코드 직접 매핑 |
| conditional breakpoint | DevTools UI | `condition <id> <expr>` |
| 변수 검사 | Scope 패널, Console | `print`, `locals`, `args` |
| 스레드/goroutine | 해당 없음 (싱글 스레드) | `goroutines`, `goroutine <id>` |
| 테스트 디버깅 | `jest --inspect` | `dlv test` |
| 원격 디버깅 | `--inspect=host:port` | `--headless --listen=:port` |
| printf 디버깅 | `console.log` | `fmt.Println`, `log.Printf` |

Go 디버깅의 가장 큰 차이는 goroutine이다. 싱글 스레드인 Node.js에서는 실행 흐름이 하나이므로 디버깅이 비교적 단순하다. Go에서는 여러 goroutine이 동시에 실행되고, 각각의 상태를 개별적으로 추적해야 한다. delve는 이를 위해 만들어진 도구다. IDE와 연동하면 Node.js에서 VS Code로 디버깅하던 경험을 거의 그대로 가져올 수 있다.
