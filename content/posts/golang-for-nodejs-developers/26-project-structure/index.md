...
Go 프로젝트가 커지면 디렉토리를 어떻게 나눌지 고민이 시작된다. Go는 커뮤니티가 수렴한 관례가 있어서 팀마다 다른 구조를 쓰는 상황이 적다. 코드 포매팅과 린팅도 마찬가지다. 논쟁 대신 도구가 결정한다.
...
`src/index.ts`가 `cmd/server/main.go`에, `src/lib/`이 `pkg/`에, `src/modules/`이 `internal/`에 대응한다고 생각하면 된다. 이 레이아웃이 강제는 아니지만, 대부분의 오픈소스 프로젝트가 따르고 있어 코드를 처음 보는 사람도 구조를 바로 파악할 수 있다.
...
`package.json`의 `bin` 필드와 달리, 각 `cmd/` 하위 디렉토리가 독립적인 바이너리를 생성한다.
...
`package.json`의 `exports` 필드나 ESLint 규칙으로 비슷한 제한을 흉내 낼 수 있지만, Go의 `internal`은 컴파일러 수준에서 강제된다는 점이 근본적으로 다르다.

## naming convention — 간결한 이름

Go는 이름을 짧게 짓는 문화가 있다. `utils/`, `helpers/`, `common/` 같은 이름은 권장되지 않는다.
...
`utils/string.ts`, `utils/date.ts`, `utils/validation.ts`처럼 성격이 다른 코드가 한 곳에 쌓이는 대신, 처음부터 `strings`, `time`, `validate` 등 구체적인 패키지로 나눈다.
...
`import { createUser, getUserById }` 같은 긴 이름 대신, 패키지명이 접두사 역할을 하므로 함수명이 짧아진다. `http.Get`, `json.Marshal`, `fmt.Println` 등 표준 라이브러리가 이 패턴을 따른다.

## gofmt와 goimports — 포매팅은 논쟁 대상이 아니다

Prettier를 설정할 때 탭이냐 스페이스냐, 세미콜론 유무, 줄 길이, trailing comma 등을 논의한다. Go에서 이런 논의는 없다.
...
ESLint의 기본 규칙과 비슷하지만, 별도 설치 없이 Go에 포함되어 있다.
...
여러 린터를 하나씩 실행하는 것은 번거롭다. `golangci-lint`는 수십 개의 린터를 통합 실행하는 도구다. ESLint가 플러그인들을 묶어 실행하는 것과 같은 위치다:
...
`.eslintrc`와 같은 역할이지만, parser, plugin, extends, rules를 조합하는 복잡함이 없다. 린터 목록에서 활성화할 항목을 고르면 된다.
...
ESLint + Prettier 충돌 같은 문제가 없다 -- gofmt와 린터가 처음부터 독립적이다.
...
`golangci-lint`는 공식 GitHub Action을 제공하므로 설정이 간단하다.
