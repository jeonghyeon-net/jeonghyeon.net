...
Node.js 생태계에서 타입 안전성을 원하면 TypeScript를 도입한다. 하지만 TypeScript는 선택적이다. `any`를 남용할 수 있고, `@ts-ignore`로 검사를 끌 수 있다. 런타임에는 타입 정보가 사라진다. JavaScript로 트랜스파일된 뒤 실행되기 때문이다.
