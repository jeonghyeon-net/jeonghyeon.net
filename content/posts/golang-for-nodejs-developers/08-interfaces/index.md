...
TypeScript에서 interface를 구현할 때 `implements`를 명시하는 것이 일반적이다:
...
TypeScript에서는 interface가 커도 큰 문제가 없다. 구조적 타이핑이 property와 method를 모두 체크하므로 interface가 크면 의도치 않은 타입이 끼어들 여지가 적고, `implements`로 의도를 명시할 수도 있다. Go에서는 interface가 클수록 암묵적으로 만족하는 타입이 줄어들고, interface의 장점이 희석된다.
