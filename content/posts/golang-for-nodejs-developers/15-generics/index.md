...
| 개념 | TypeScript | Go |
|---|---|---|
| 타입 파라미터 문법 | `<T>` | `[T constraint]` |
| 기본 constraint | `extends` | interface |
| 모든 타입 허용 | `<T>` (기본) | `[T any]` |
| 비교 가능 타입 | 별도 없음 | `[T comparable]` |
| 순서 비교 가능 | 별도 없음 | `[T cmp.Ordered]` |
| union constraint | `T extends A \| B` | `A \| B` (interface 내) |
| 타입 erasure | 있음 (런타임에 사라짐) | 없음 (컴파일 타임 구체화) |
| 메서드 타입 파라미터 | 가능 | 불가 |
| constraint 표현력 | 매우 높음 | 의도적으로 단순 |

Go의 제네릭은 10년 넘게 논의 끝에 추가되었다. 그 기간만큼 보수적으로 설계되었다. TypeScript처럼 타입 수준의 프로그래밍을 하는 것이 아니라, 코드 중복을 제거하는 실용적 도구로 자리 잡았다. `slices`, `maps`, `cmp` 같은 표준 라이브러리가 제네릭의 가장 좋은 사용 예시다. 복잡한 제네릭 코드를 작성하기 전에 interface로 충분한지, 구체적인 타입으로 2-3번 쓰는 것이 더 명확한지 먼저 고려하는 것이 Go 스타일이다.
