...
Go에서 에러는 값이다. 던지고 잡는 것이 아니라 반환하고 검사한다. try/catch에서 이 방식으로의 전환은 단순한 문법 변화가 아니라 멘탈 모델의 전환이다.
...
TypeScript의 `strictNullChecks`를 켜면 상황이 나아진다. 반환 타입을 `User | null`로 명시하면 null 체크 없이 property에 접근하는 코드를 컴파일러가 잡아준다:
...
const user = getUser(123);
user.name; // 컴파일 에러: 'user' is possibly 'null'

if (user) {
  user.name; // OK — null이 아님이 확인됨
}
```

이것만으로도 상당한 안전 장치다. 하지만 `strictNullChecks`는 프로젝트 설정에 의존하는 opt-in이고, `as` 단언이나 `!` non-null assertion으로 우회할 수 있다. 무엇보다 에러 처리 자체를 강제하지는 않는다 — null이 아닌 경우의 에러 정보(왜 null인지)를 전달할 표준 방법이 없다.
...
`Error() string` 메서드 하나만 있으면 error다(08편의 implicit satisfaction).
...
Node.js에서는 에러 객체의 property(`.code === "ENOENT"`)로 구분하지만, Go는 에러 값 자체를 비교한다.
...
`errors.Is`가 값 비교라면, `errors.As`는 타입 비교다. 08편의 type assertion과 같은 원리지만, wrapping된 에러 체인 전체를 탐색한다.
...
Node.js의 에러 코드(`err.code === "ECONNREFUSED"`)가 문자열 비교인 반면, Go의 sentinel error는 값 비교이므로 타입 안전하다.
...
custom error 타입은 에러에 구조화된 정보를 담을 때 유용하다. HTTP 상태 코드, 실패한 필드 이름, 재시도 가능 여부 등을 에러 자체에 포함할 수 있다. JavaScript의 `instanceof` 검사와 달리, `errors.As`는 wrapping된 에러 체인을 따라가므로 에러가 여러 번 wrapping되어도 원본 타입을 찾을 수 있다.
...
`if err != nil`의 장황함이 대가로 주는 것이 있다. 에러 경로가 코드에 명시적으로 드러나고, 어떤 함수가 실패할 수 있는지 반환 타입만 보면 알 수 있고, 에러를 값으로 다루므로 유연한 처리가 가능하다. Tony Hoare가 "보이지 않는 null"의 위험을 경고했듯이, Go는 "보이지 않는 에러 경로"를 제거하는 쪽을 택했다.

## TypeScript 생태계의 대안

TypeScript 생태계에서도 이 방향을 탐색하는 움직임이 있다. neverthrow, ts-results 같은 라이브러리가 `Result<T, E>` 타입을 제공한다:
...
Go의 `(value, error)` 반환 패턴과 같은 아이디어다. 차이는 Go에서는 이것이 언어 전체의 관례이고 표준 라이브러리가 모두 따르는 반면, TypeScript에서는 서드파티 라이브러리에 의존하며 생태계 전반에 채택되지는 않았다는 점이다.
