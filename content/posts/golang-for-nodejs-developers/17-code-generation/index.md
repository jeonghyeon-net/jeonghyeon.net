...
Go에는 enum이 없다. 06편에서 다뤘듯이 `iota`로 상수를 정의한다:
...
| 개념 | Node.js/TypeScript | Go |
|---|---|---|
| 동적 행위 | decorator, reflection, proxy | 코드 생성 |
| 생성 트리거 | `postinstall`, 빌드 스크립트 | `go generate` (명시적 실행) |
| 생성 선언 위치 | `package.json`, 별도 설정 파일 | 소스 코드 주석 (`//go:generate`) |
| enum 문자열 변환 | TypeScript enum은 기본 지원 | `stringer`로 생성 |
| mock 생성 | `jest.mock()` (런타임) | `mockgen` (컴파일 타임) |
| API 코드 생성 | OpenAPI codegen, GraphQL codegen | `protoc` (protobuf/gRPC) |
| 생성된 코드 | 보통 커밋하지 않음 | 커밋함 (표준 관행) |
| 빌드 의존성 | 생성 도구가 빌드에 필요 | Go 컴파일러만으로 빌드 가능 |

Go의 코드 생성 철학은 "명시적인 것이 암묵적인 것보다 낫다"를 극단까지 밀어붙인 결과다. decorator가 런타임에 조용히 수행하는 변환을 Go는 실제 소스 코드로 만들어 저장소에 넣는다. 생성된 코드를 읽을 수 있고, 디버깅할 수 있고, git에서 변경을 추적할 수 있다. 마법은 없지만 투명하다.
