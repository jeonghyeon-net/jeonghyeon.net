...
여러 goroutine의 완료를 기다리는 가장 간단한 방법이다:
...
func main() {
    ...
    wg.Wait() // 모든 goroutine이 Done()을 호출할 때까지 대기
}
```

`Add(1)`로 카운터를 증가시키고, `Done()`으로 감소시킨다. `Wait()`는 카운터가 0이 될 때까지 블로킹된다. Node.js의 `Promise.all`과 비슷한 역할이다:
...
차이점: `Promise.all`은 하나가 reject되면 즉시 reject된다. `WaitGroup`은 에러 개념이 없다. 에러를 모으려면 channel을 조합하거나 `errgroup` 패키지를 쓴다:
...
func main() {
    ...
    if err := g.Wait(); err != nil {
        fmt.Println("error:", err) // error: task 3 failed
    }
}
```

`errgroup.Group`은 `golang.org/x/sync/errgroup` 패키지에 있다. `Go` 메서드로 goroutine을 시작하고, `Wait`가 첫 번째 에러를 반환한다.
...
Go의 모델은 "동시성을 언어의 기본 요소로 제공한다." goroutine과 channel이 변수나 함수만큼 자연스러운 도구다. 대신 race condition이라는 새로운 종류의 버그를 다뤄야 한다. `go run -race`로 race condition을 감지할 수 있다. 이 주제는 이후 편에서 더 깊이 다룬다.

## 실전 패턴: fan-out/fan-in

여러 goroutine에 작업을 분배(fan-out)하고, 결과를 하나의 channel로 모으는(fan-in) 패턴이다:

```go
func worker(id int, jobs <-chan int, results chan<- int) {
    for j := range jobs {
        time.Sleep(time.Duration(j) * 100 * time.Millisecond) // 작업 시뮬레이션
        results <- j * 2
    }
}
...
9개의 작업을 3개의 worker가 나눠서 처리한다. jobs channel로 작업을 분배하고, results channel로 결과를 모은다. Node.js에서 비슷한 패턴을 구현하려면 작업 큐 라이브러리를 쓰거나 직접 구현해야 한다. Go에서는 channel과 goroutine만으로 자연스럽게 표현된다.

## 정리

| 개념 | Node.js | Go |
|---|---|---|
| 동시 실행 단위 | Promise / async function | goroutine |
| 통신 | 콜백, 이벤트, Promise | channel |
| 여러 작업 대기 | `Promise.race`, `Promise.all` | `select`, `sync.WaitGroup` |
| timeout | `setTimeout` + `Promise.race` | `select` + `time.After` |
| worker pool | `worker_threads` | goroutine + channel |
| 에러 수집 | `Promise.allSettled` | `errgroup` |
| 경량 실행 단위 비용 | Promise 자체는 가벼움 | goroutine: 2KB 스택, 50~100ns switch |
| 병렬 실행 | `cluster`, `worker_threads` | 기본 내장 |

Go의 동시성 모델은 이 시리즈에서 가장 큰 패러다임 전환이다. Node.js에서는 "어떻게 비동기를 관리하는가"가 핵심 질문이었다면, Go에서는 "어떻게 독립적인 실행 단위를 구조화하고, 이들 사이의 통신을 설계하는가"가 핵심 질문이다. callback 지옥이나 Promise 체인과 싸울 필요 없이, 순차적으로 보이는 코드가 동시에 실행된다.
