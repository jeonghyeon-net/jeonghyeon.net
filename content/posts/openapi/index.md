...
`queryKey`는 캐시를 구분하는 식별자임. `["user", id]`라는 키로 데이터를 가져오면 그 결과가 이 키에 저장됨. 다른 컴포넌트 10개가 같은 `["user", "123"]`으로 `useQuery`를 써도 API 호출은 한 번만 일어나고 결과를 공유함.

배열인 이유가 있음. 계층 구조를 쓸 수 있어서임. `invalidateQueries({ queryKey: ["user"] })`를 하면 `["user", "123"]`, `["user", "456"]` 등 `["user"]`로 시작하는 캐시가 전부 날아감. mutation 후에 관련 데이터를 한꺼번에 갱신할 때 유용함.

### 캐싱이랑 동기화

여기서 의문이 하나 생길 수 있음. 캐시가 클라이언트에 있으면, 다른 사용자가 서버 데이터를 바꿨을 때 내 화면에는 반영이 안 되는 거 아님?

맞음. TanStack Query는 클라이언트 캐시임. 근데 이걸 해결하는 기본 동작이 내장되어 있음.

**기본값이 `staleTime: 0`, `refetchOnWindowFocus: true`임.** 이게 무슨 뜻이냐면, 데이터를 가져오는 순간 바로 stale(낡은) 상태가 되고, 사용자가 다른 탭 갔다가 돌아오면 자동으로 refetch함. 별도 설정 안 해도 탭 전환만으로 최신 데이터가 반영됨.

실무에서는 대부분 이 기본 설정으로 충분함. 사용자가 탭 돌아오면 최신 데이터 가져오고, 페이지 이동했다가 돌아와도 다시 가져옴.

실시간성이 좀 더 필요한 경우(채팅 목록, 알림 카운트, 주문 현황 같은 거)는 `refetchInterval`로 polling함. `refetchInterval: 5000`이면 5초마다 자동으로 가져옴. 대부분의 프로덕션 앱이 이 선에서 해결됨.

WebSocket은 진짜 실시간이 필수인 경우(채팅 메시지, 주식 시세, 협업 에디터)에만 씀. 서버가 "데이터 바뀌었음"이라고 알려주면 그때 `invalidateQueries`를 호출하는 패턴임. 구현 복잡도가 확 올라가서 필요하지 않으면 안 쓰는 게 나음.

### 그 외 기능들

**staleTime**은 위에서 설명했고, 이걸 늘리면 refetch 빈도를 줄일 수 있음. `staleTime: 60 * 1000`이면 1분 안에는 같은 데이터를 다시 안 가져옴. 자주 바뀌지 않는 데이터(사용자 프로필 같은 거)에 적합함.

**gcTime**(구 cacheTime)은 캐시가 메모리에 남아있는 시간임. 컴포넌트가 언마운트된 후에도 이 시간 동안은 캐시가 유지돼서, 다시 마운트되면 API 호출 없이 바로 보여줌. 기본값은 5분임.

**placeholderData**는 이전 데이터를 유지하면서 새 데이터를 가져올 때 쓰는 거임. 페이지네이션에서 2페이지로 넘어갈 때 빈 화면 대신 1페이지 데이터를 유지하다가 2페이지 데이터가 오면 교체하는 식으로 동작함.

**optimistic updates**는 `onMutate`에서 캐시를 먼저 업데이트하고 서버 요청이 실패하면 `onError`에서 롤백하는 패턴임. 좋아요 버튼 같은 데서 쓰면 사용자 입장에서 반응이 즉각적으로 느껴짐.

**useInfiniteQuery**는 무한 스크롤 구현용임. `getNextPageParam`으로 다음 페이지 정보를 관리하고 `fetchNextPage`로 추가 데이터를 가져옴.
