...
func pageTemplate(cfg markdownutil.SiteConfig, title, description, pageURL, siteURL, imageURL, headerHTML, bodyHTML, footerHTML string) string {
	...
	b.WriteString("  <meta name=\"msvalidate.01\" content=\"F886743B6BEC1975804AFAF0B7D1F5A8\">\n")
...
func Render(contentDir, distDir string) error {
	...
	// Read layout files
	headerSrc, err := os.ReadFile(filepath.Join(contentDir, "header", "index.md"))
	if err != nil {
		return fmt.Errorf("read header/index.md: %w", err)
	}
	footerSrc, err := os.ReadFile(filepath.Join(contentDir, "footer", "index.md"))
	if err != nil {
	...
	err = filepath.WalkDir(contentDir, func(absPath string, d os.DirEntry, walkErr error) error {
		...
		// Layout directories: skip .md files already read above, but copy assets.
		if slashRel == "header" || strings.HasPrefix(slashRel, "header/") ||
