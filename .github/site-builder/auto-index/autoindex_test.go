...
func TestGenerate_SkipsLayoutDirs(t *testing.T) {
	generated, err := Generate("testdata/layout-dirs")
	if err != nil {
		t.Fatalf("Generate error: %v", err)
	}
	...
	for _, path := range []string{"header/index.md", "footer/index.md"} {
		if _, ok := generated[path]; ok {
			t.Errorf("expected layout file %s to be skipped", path)
		}
	}
}

func TestGenerate_SkipsRepositoryDirectories(t *testing.T) {
	root := t.TempDir()
	for _, dir := range []string{
		".git/objects/aa",
		".github/site-builder/testdata/case",
		"dist/cache",
		"posts/my-post",
	} {
	...
	for path := range generated {
		if strings.HasPrefix(path, ".git/") || strings.HasPrefix(path, ".github/") || strings.HasPrefix(path, "dist/") {
			t.Errorf("expected repository directory to be skipped, generated %s", path)
		}
	}
	if _, ok := generated["posts/index.md"]; !ok {
		t.Errorf("expected site directory to remain discoverable, got keys: %v", keys(generated))
	}
}
